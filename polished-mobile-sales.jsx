...
const msData = () => {
  try { const s = localStorage.getItem("miwa.sales.v9"); if (s) { const v = JSON.parse(s); if (Array.isArray(v) && v.length) return v; } } catch (e) {}
  return window.MIWA_SEED_DATA || [];
};

const MSales = ({ registerHeader, registerFab }) => {
  const [rows] = React.useState(msData);
  const [month, setMonth] = React.useState("");
  const [store, setStore] = React.useState("");

  React.useEffect(() => {
    registerHeader && registerHeader({ title: "売上レポート", sub: "店舗別・月次" });
    registerFab && registerFab(null);
  }, []);
  ...
  return (
    <div>
      ...
      <div className="m-pr-note">数値はPC版売上レポートと共有。詳細な編集・CSV取込はPC版で行えます。</div>
      <div style={{ height: 16 }}></div>
    </div>
  );
};
